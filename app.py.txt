# ---------------------------------------------------------------------------
# Guild Analyzer Pro - Admin Notes & Export Edition  (May 2026)
# ---------------------------------------------------------------------------
...
def clean_excel(df):
    ...
    for col in expected:
        if col not in df.columns:
            df[col] = 0
    df = df[expected]
    for c in ["Might","Old Might","Might Difference","Kills",
              "Old Kills","Kills Difference"]:
...
if not st.session_state.logged_in:
    ...
    if act=="Kayıt Ol":
        if st.button("Kayıt Ol"):
            st.success("Kayıt başarılı.") if register_user(u,p) else st.error("Kullanıcı mevcut.")
    else:
        if st.button("Giriş"):
            ok,adm=verify_user(u,p)
            if ok: st.session_state.update(logged_in=True,user=u,is_admin=adm); st.rerun()
            else: st.error("Hatalı kullanıcı veya parola.")
else:
...
def generate_weekly_report(admin_user="Admin"):
    ...
    for _,row in top5.iterrows():
        pdf.cell(100,8,txt=f"{row['name']}: {int(row['Kills Difference']):,}",ln=True)
    pdf.ln(5)

    pdf.set_font("Arial",'B',12)
    pdf.cell(100,10,"Lowest 5",ln=True)
    pdf.set_font("Arial", size=11)
...
if st.session_state.is_admin:
    ...
    # --------- GUILD ANALYZER ----------
        ...
        if not data.empty:
            ...
            if filt.empty: st.info("Bu tarihte veri yok.")
            else:
                c1,c2=st.columns(2)
    ...
    # --------- GF POINT ----------
        ...
        else: st.info("GF verisi bulunamadı.")

    # --------- ÜYELER + PROFİL DETAY + NOTE / EXPORT ----------
    elif menu=="Üyeler":
        conn=sqlite3.connect(DB_PATH)
        ...
        if st.session_state.is_admin and not mem.empty:
            ...
            if not p_data.empty:
                ...
                first=p_data["upload_date"].min()
                last=p_data["upload_date"].max()
                ...
                # --- Export buttons ---
                plt.tight_layout()
                fig.savefig("player_chart.png")
                with open("player_chart.png","rb") as img:
                    st.download_button("📸 PNG olarak indir", img,
                        file_name=f"{selected}_chart.png")
                ...
                with open("player_chart.pdf","rb") as f:
                    st.download_button("📄 PDF olarak indir", f,
                ...
                st.info("Üye için kayıt bulunamadı.")
