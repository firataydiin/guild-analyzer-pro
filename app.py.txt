...
def register_user(username, password, is_admin=False):
    ...
    cur.execute("INSERT INTO users VALUES (?, ?, ?)", (username, hashed, int(is_admin)))
    conn.commit()
    conn.close()
    return True
...
def login_user(username, password):
    ...
    if not user:
        return False, False
    stored_hash, is_admin = user[1], user[2]
    if bcrypt.checkpw(password.encode(), stored_hash.encode()):
        return True, bool(is_admin)
    return False, False

# --- APP ---
st.set_page_config(page_title="Guild Analyzer Pro (Admin Enabled)", layout="wide")
init_db()
...
if not st.session_state.logged_in:
    ...
    if choice == "Kayıt Ol":
        is_admin = st.checkbox("Bu kullanıcı admin olsun (yalnızca ilk oluşturucu seçsin!)")
        if st.button("Hesap Oluştur"):
            if register_user(username, password, is_admin=is_admin):
    ...
    st.title("⚔️ Guild Analyzer Pro (Admin Yönetimiyle)")

    uploaded = st.file_uploader("Yeni Excel Yükle (.xlsx)", type=["xlsx"])
    if uploaded:
    ...
    if st.session_state.is_admin:
        ...
        row_to_delete = st.number_input("Silinecek ID:", min_value=0, step=1)
        if st.button("Sil"):
            delete_row(row_to_delete)
            st.warning(f"ID {row_to_delete} silindi.")
            st.experimental_rerun()
    else:
        st.info("🔒 Sadece admin kullanıcılar kayıt silebilir.")
