# ---------------------------------------------------------------------------
# Guild Analyzer Pro v3 – Weekly Visual Dashboard with UI Improvements
# ---------------------------------------------------------------------------
...
.metric-container {background:#252525;border-radius:8px;padding:10px;text-align:center;margin:5px;}
</style>
"""
LIGHT_CSS = """
<style>
body, .stApp {background-color:#fafafa; color:#111;}
.metric-container {background:#f0f0f0;border-radius:8px;padding:10px;text-align:center;margin:5px;}
</style>
"""

# === UI ===
st.set_page_config("Guild Analyzer Pro v3", layout="wide")
...
if not st.session_state.logged_in:
    st.title("⚔️ Guild Analyzer Pro v3")
    ...
    if menu=="Dashboard":
        ...
        if data.empty:
            ...
            haftalar=sorted(data["Hafta"].unique().tolist())
            ...
            # Renkli metrik kartlar
            st.markdown("<div class='metric-container'>🩸 <b>Toplam Kill Sayısı:</b> {:,.0f}</div>".format(total_k), unsafe_allow_html=True)
            st.markdown("<div class='metric-container'>💪 <b>Toplam Might:</b> {:,.0f}</div>".format(total_m), unsafe_allow_html=True)
            st.markdown("<div class='metric-container'>👥 <b>Aktif Oyuncu:</b> {}</div>".format(aktif), unsafe_allow_html=True)

            st.markdown("### 🏆 En Çok Kill Yapan 10 Oyuncu")
            top10=haftalik.sort_values("kills_diff",ascending=False).head(10)
            fig,ax=plt.subplots(figsize=(8,4))
            colors=plt.cm.plasma(top10["kills_diff"]/max(top10["kills_diff"]))
            ax.barh(top10["name"],top10["kills_diff"].astype(int),color=colors)
            ax.invert_yaxis()
            ax.set_xlabel("Kills Δ")
            for i, v in enumerate(top10["kills_diff"].astype(int)):
                ax.text(v+1, i, f"{v:,}", va='center')
            st.pyplot(fig)

            st.markdown("### 📋 Tüm Oyuncular (Haftalık Toplam Kill)")
            haftalik=haftalik.sort_values("kills_diff",ascending=False).reset_index(drop=True)
            haftalik["kills_diff"]=haftalik["kills_diff"].astype(int)
            st.dataframe(haftalik.style.background_gradient(cmap="Greens",subset=["kills_diff"]))
